%% MAIN RUN CODE FOR ME495 HAPTICS PSYCHOPHYSICS PROJECT (DESCENDING METHOD)
...
% Min and baseline test wavelengths
w1_min = 0.001;
w1_bl = 0.2;
...
T1_guess = 1.2*(w1_bl-threshold1);
T2_guess = 1.2*(w2_bl-threshold2);
...
[QUEST1,stim1_dB] = Quest_init_in(threshold1,T1_guess,w1_bl,w1_min);
[QUEST2,stim2_dB] = Quest_init_in(threshold2,T2_guess,w2_bl,w2_min);
...
% Tests 1 and 2 are both active
    if and(flag1,flag2)
        ...
        if test == 1
            ...
            [stim1_dB_test_mat(n1),p1_test_mat(n1),QUEST1_mat(n1+1,:),resp1_mat(n1)] = Quest_main_in(send,trial_count,n1,threshold1,stim1_dB,QUEST1_mat(n1,:),w1_min,w1_bl);
            n1 = n1+1;
        % Test 2
        else
            disp([sprintf('\n'),'(Test 2, Trial ',num2str(n2),')'])
            [stim2_dB_test_mat(n2),p2_test_mat(n2),QUEST2_mat(n2+1,:),resp2_mat(n2)] = Quest_main_in(send,trial_count,n2,threshold2,stim2_dB,QUEST2_mat(n2,:),w2_min,w2_bl);
            n2 = n2+1;
        end
...
% Test 1 Plots
    if n1 > 1;
        ...
        subplot(3,1,1)
        threshold_vec1 = (w1_min-w1_bl)*10.^(stim1_dB_test_mat(1:n1-1)/20);
        plot(1:n1-1,threshold_vec1)
        axis([1,n1,w1_min-w1_bl,0]);
...
% Test 2 Plots
    if n2 > 1;
        ...
        % Wavelength threshold vs trial #
        subplot(3,1,1)
        threshold_vec2 = (w2_min-w2_bl)*10.^(stim2_dB_test_mat(1:n2-1)/20);
        plot(1:n2-1,threshold_vec2)
        axis([1,n2,w2_min-w2_bl,0]);
...
%% Trim matrices to size for output
